#!/bin/sh
#*=====================================================================*/
#*    .../diffusion/article/flt/artifact/scripts/hop_fltcell.sh        */
#*    -------------------------------------------------------------    */
#*    Author      :  Manuel Serrano                                    */
#*    Creation    :  Thu Oct  3 08:33:46 2024                          */
#*    Last change :  Thu Nov  7 07:34:26 2024 (serrano)                */
...
if [ "$action " = "install " -o "$action " = "all " ]; then
  ...
  if [ ! -f $installdir/bin/hop ]; then
    if [ ! -f ${downloaddir}/config.status ]; then
      configure --prefix=${installdir} --bigloo=${ROOT}/install/bigloo_fltcell/bin/bigloo || exit 1
    fi
    make_compile || exit 1
    make_install
  fi
