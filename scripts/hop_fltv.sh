#!/bin/sh
#*=====================================================================*/
#*    serrano/diffusion/article/flt/artifact/scripts/hop_fltv.sh       */
#*    -------------------------------------------------------------    */
#*    Author      :  Manuel Serrano                                    */
#*    Creation    :  Thu Oct  3 08:33:46 2024                          */
#*    Last change :  Thu Nov 14 08:33:10 2024 (serrano)                */
...
if [ "$action " = "install " -o "$action " = "all " ]; then
  ...
  if [ ! -f $installdir/bin/hop ]; then
    if [ ! -f ${downloaddir}/config.status ]; then
      configure --prefix=${installdir} --bigloo=${ROOT}/install/bigloo_fltv/bin/bigloo || exit 1
    fi
    make_compile || exit 1
    make_install
  fi
