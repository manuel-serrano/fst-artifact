#!/bin/sh
#*=====================================================================*/
#*    serrano/diffusion/article/flt/artifact/scripts/bigloo_nan.sh     */
#*    -------------------------------------------------------------    */
#*    Author      :  Manuel Serrano                                    */
#*    Creation    :  Thu Oct  3 08:33:46 2024                          */
#*    Last change :  Tue Jun 24 15:46:02 2025 (serrano)                */
...
if [ "$action " = "install " -o "$action " = "all " ]; then
  ...
  if [ ! -f $installdir/bin/bigloo ]; then
    if [ ! -f ${downloaddir}/Makefile.config ]; then
      configure $bigloo_configure_opts --prefix=${installdir} --nan-tagging || exit 1
    fi
    make_compile || exit 1
    make_install
  fi
